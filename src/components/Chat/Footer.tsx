...
import { Laugh, Mic, Paperclip, SendHorizontal, Trash, X } from "lucide-react";
...
export const Footer = ({ onSendMessage }: Props) => {
    ...
    const handleSendMessage = () => {
        if (messageValue.trim() || messageAttachment) {
                ...
                {isRecording ? (
                    <div className="flex items-center gap-5">
                        <Button variant="ghost" size="icon" title="Para Gravação" onClick={handleDeleteRecording}>
                            <Trash className="h-5 w-5 text-slate-500 dark:text-slate-300" />
                        </Button>
                        ...
                        <Button className="ml-6" size="sm" title="Enviar Mensagem de Voz" onClick={handleSendRecording}>
                            <SendHorizontal className="text-slate-100" />
                        </Button>
                    </div>
                ) : (
                    <>
